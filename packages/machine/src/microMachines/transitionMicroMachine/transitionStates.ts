import {TransitionMachine, MicroSchema} from "./index";
...
export const fsa: MicroSchema<SubStateId, SubContext> = {
  initial: 'from',
  // final: ['to'],
  states: {
...
async function run() {
  ...
  const m = new TransitionMachine<SubStateId, ResolvedTransitionContext<any, any>>({schema: fsa, context});
  await m.exec();
  // console.log(m);
}
